...
class ConversationManager:
    ...
    def resolve_vague_query(self, user_id: str, query: str) -> str:
        ...
        # Check for explicit legal reference (e.g., "Điều 19")
        legal_reference_match = re.search(r'điều\s+(\d+[a-z]?)', query.lower())
        if legal_reference_match:
            article = legal_reference_match.group(1)
            logger.info(f"🔗 Legal reference detected: Điều {article}")
            return f"quy định tại Điều {article} Luật Xuất cảnh, nhập cảnh"
...
class SituationMatcher:
    ...
    def match(self, resolved_query: str, intent_analysis: dict, domain: str = "xuatnhapcanh") -> str:
        """Simple JSON matching - chỉ exact match question_patterns"""
        try:
            ...
            if 'procedures' in data:
                for procedure in data['procedures']:
                    # Check main question_patterns
                    patterns = procedure.get('question_patterns', [])
                    for pattern in patterns:
                        if pattern.lower().strip() == query_lower:
                            logger.info(f"✅ Exact match procedure: {procedure.get('procedure_name', '')}")
                            return procedure.get('base_response', {}).get('content', '')
                    
                    # Check situation question_patterns
                    situations = procedure.get('situations', [])
                    for situation in situations:
                        situation_patterns = situation.get('question_patterns', [])
                        for pattern in situation_patterns:
                            if pattern.lower().strip() == query_lower:
                                logger.info(f"✅ Exact match situation: {situation.get('situation_id', '')}")
                                return situation.get('response', {}).get('content', '')
            
            # 2. Check legal_situations
            if 'legal_situations' in data:
                for legal_situation in data['legal_situations']:
                    patterns = legal_situation.get('question_patterns', [])
                    for pattern in patterns:
                        if pattern.lower().strip() == query_lower:
                            logger.info(f"✅ Exact match legal: {legal_situation.get('situation_id', '')}")
                            return legal_situation.get('response', {}).get('content', '')
