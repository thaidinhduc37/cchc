...
const Register = () => {
    ...
    const validateField = (name, value) => {
        switch (name) {
            case 'fullName':
                return !value.trim() ? 'Họ và tên không được để trống' : 
            ...
            case 'phone':
                return !value.trim() ? 'Số điện thoại không được để trống' :
                       !/^[0-9]{10,11}$/.test(value.replace(/\s/g, '')) ? 'Số điện thoại không hợp lệ' : '';
            case 'password':
                return !value.trim() ? 'Mật khẩu không được để trống' : 
                       value.length < 8 ? 'Mật khẩu phải có ít nhất 8 ký tự' : 
                       !/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/.test(value) ? 'Mật khẩu phải chứa chữ hoa, chữ thường và số' : '';
            case 'confirmPassword':
                return !value.trim() ? 'Vui lòng xác nhận mật khẩu' :
                        ...
                        <div className={cx('input-group')}>
                            <label className={cx('checkbox-wrapper', 'terms-wrapper')}>
                                ...
                                <span className={cx('checkbox-label')}>
                                    Tôi đồng ý với 
                                    <button type="button" className={cx('terms-link')}>Điều khoản sử dụng</button>
                                    và 
                                    <button type="button" className={cx('terms-link')}>Chính sách bảo mật</button>
                                </span>
                            </label>
                            {errors.agreeTerms && (
                                <div className={cx('error-message')}>{errors.agreeTerms}</div>
                            )}
                        </div>
