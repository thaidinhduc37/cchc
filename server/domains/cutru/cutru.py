...
file_path = os.path.join(os.path.dirname(__file__), "cutru.json")
with open(file_path, "r", encoding="utf-8") as f:
    DATA = json.load(f)["entries"]

def get_interaction_flow():
    flow_path = os.path.join(os.path.dirname(__file__), "interaction_flow.json")
...
def get_next_step(current_id, user_input=None):
    ...
    if step["type"] == "choice":
        ...
        matched = next((opt for opt in options if normalized_input in normalize_text(opt["label"]) or normalize_text(opt["label"]) in normalized_input), None)
        if matched:
...
def get_domain_context():
    ...
    return (
        "Bạn là trợ lý ảo hỗ trợ công dân thực hiện các thủ tục cư trú, tạm trú, hộ khẩu.\n\n"
