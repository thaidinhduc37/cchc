...
"""
Vector RAG System Test Suite - FIXED for server/service/vector_rag structure
"""
...
# 🔥 CORRECT PATH setup for server/service/vector_rag
...
print(f"📁 Expected RAG path: {os.path.join(server_dir, 'service', 'vector_rag')}")

# Import RAG components - FIXED for service/vector_rag structure
RAG_AVAILABLE = False
rag_import_error = None

try:
    print("🔍 Importing RAG Engine from service/vector_rag...")
    
    # Primary import path: server/service/vector_rag
    try:
        from service.vector_rag.rag_engine import RAGEngine
        from service.vector_rag.rag_config import config
        print("✅ RAG imports successful (service.vector_rag)!")
        RAG_AVAILABLE = True
    except ImportError as e1:
        print(f"⚠️ service.vector_rag import failed: {e1}")
        
        # Try adding service/vector_rag to path directly
        try:
            vector_rag_path = os.path.join(server_dir, 'service', 'vector_rag')
            if vector_rag_path not in sys.path:
                sys.path.insert(0, vector_rag_path)
            
            from rag_engine import RAGEngine
            from rag_config import config
            print("✅ RAG imports successful (direct path)!")
            RAG_AVAILABLE = True
        except ImportError as e2:
            print(f"⚠️ Direct import failed: {e2}")
            
            # Last attempt - try all possible paths
            try:
                # Add service directory to path
                service_path = os.path.join(server_dir, 'service')
                if service_path not in sys.path:
                    sys.path.insert(0, service_path)
                
                from vector_rag.rag_engine import RAGEngine
                from vector_rag.rag_config import config
                print("✅ RAG imports successful (service path)!")
                RAG_AVAILABLE = True
...
if not RAG_AVAILABLE:
    ...
    # Check if service/vector_rag directory exists
    service_rag_dir = os.path.join(server_dir, 'service', 'vector_rag')
    if os.path.exists(service_rag_dir):
        files = os.listdir(service_rag_dir)
        python_files = [f for f in files if f.endswith('.py')]
        print(f"   ✅ service/vector_rag directory exists with files: {python_files}")
        
        # Check for required files
        required_files = ['rag_engine.py', 'rag_config.py']
        ...
        print(f"   ❌ service/vector_rag directory not found: {service_rag_dir}")
        
        # Check alternative paths
        alt_paths = [
            os.path.join(server_dir, 'services', 'vector_rag'),
            os.path.join(server_dir, 'vector_rag'),
            os.path.join(server_dir, 'src', 'vector_rag')
        ]
    ...
    print(f"   ❌ Last error: {rag_import_error}")
...
class RAGSystemTest:
    ...
    def get_test_queries(self):
        """Get test queries for RAG system"""
        return [
            # Basic passport queries
            {
                'query': 'Ai được cấp hộ chiếu phổ thông?',
                'category': 'basic',
                'expected_keywords': ['công dân việt nam', 'đủ 14 tuổi', 'hộ chiếu'],
                'description': 'Basic passport eligibility question'
            },
            {
                'query': 'Hồ sơ cấp hộ chiếu gồm những gì?',
                'category': 'procedure',
                'expected_keywords': ['tờ khai', 'ảnh', 'chứng minh', 'căn cước'],
                'description': 'Passport application documents'
            },
            {
                'query': 'Thời gian cấp hộ chiếu là bao lâu?',
                'category': 'time',
                'expected_keywords': ['8 ngày', 'ngày làm việc'],
                'description': 'Passport processing time'
            },
            {
                'query': 'Lệ phí làm hộ chiếu bao nhiêu?',
                'category': 'fee',
                'expected_keywords': ['160', '320', 'đồng'],
                'description': 'Passport fees'
            },
            {
                'query': 'Trình báo mất hộ chiếu phải làm sao?',
                'category': 'procedure',
                'expected_keywords': ['trình báo', 'mất', 'công an'],
            ...
            # Legal queries
            {
                ...
                'category': 'legal',
                'expected_keywords': ['điều', 'luật', 'xuất cảnh'],
                'description': 'Legal conditions for exit'
            },
            {
            ...
            # Complex queries
            {
                ...
                'category': 'complex',
                'expected_keywords': ['trẻ em', 'dưới 14 tuổi', 'cha mẹ', 'giấy khai sinh'],
            ...
            # Visa queries
            {
                ...
                'category': 'visa',
                'expected_keywords': ['thị thực', 'visa', 'người nước ngoài'],
    ...
    try:
        ...
        if result.get('success'):
            ...
            try:
                ...
                print("⚠️ health_check method not available, using stats instead")
...
def check_directory_structure():
    ...
    # Check for different possible RAG locations
    possible_paths = [
        os.path.join(server_dir, 'service', 'vector_rag'),
        os.path.join(server_dir, 'services', 'vector_rag'),
        os.path.join(server_dir, 'src', 'service', 'vector_rag'),
        os.path.join(server_dir, 'vector_rag'),
    ]
    
    print("\n🔍 Checking possible RAG locations:")
...
async def main():
    ...
    if not RAG_AVAILABLE:
        print("\n❌ RAG system not available. Showing directory check...")
...
if __name__ == "__main__":
    # Show directory structure if RAG not available
    if not RAG_AVAILABLE:
        print("\n" + "="*60)
        check_directory_structure()
        print("="*60)
